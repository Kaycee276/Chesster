    ...
    (set, get) => ({
        ...
        if (data.success) {
          ...
          if (data.data.status === "active" && data.data.game_started_at) {
            const elapsed = Math.floor(
              (Date.now() - new Date(data.data.game_started_at).getTime()) /
                1000,
            );
      ...
      fetchGameState: async () => {
        ...
        if (data.success) {
          ...
          const isTransitionToActive =
            nextStatus === "active" && prevStatus === "waiting";
      ...
      offerDraw: async () => {
        ...
        if (!gameCode || !playerColor) return;
        const data = await api.offerDraw(
          gameCode,
          playerColor,
      ...
      const msg =
        endReason === "stalemate"
          ...
          : endReason === "draw_agreed"
            ? "Draw agreed — well played!"
            : endReason === "time"
              ? "Time's up — equal material, it's a draw!"
      ...
      const msg =
        endReason === "checkmate"
          ...
          : endReason === "resignation"
            ...
            : endReason === "time"
              ? "Time's up — you had more material!"
              : "You won!";
      ...
      const msg =
        endReason === "checkmate"
          ...
          : endReason === "resignation"
            ? "You resigned."
            : endReason === "time"
              ? "Time's up — opponent had more material."
              : "You lost.";
      addToast(msg, "error");
    }
